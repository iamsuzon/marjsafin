...
@section('title', 'Slip Registration')
    ...
    <div class="page-content">
        <div class="container-fluid">
            ...
            <div class="card">
                <div class="row">
                    <div class="col-12">
                        <h2 class="manage__title mb-25">Slip</h2>
                        ...
                        <form action="{{route('user.slip.registration')}}" method="POST" id="registration-form">
                            @csrf
                            ...
                            <div class="row g-16">
                                    ...
                                    <div class="contact-form">
                                        ...
                                        <select class="select2" name="slip_type">
                                            ...
                                            @foreach(slipType() ?? [] as $index => $slip)
                                                <option {{old('slip_type') === $index ? 'selected' : ''}} value="{{ $index }}">{{ $slip }}</option>
                                            @endforeach
                                        </select>
                                    </div>
                                </div>
                                ...
                                <div class="col-xl-3 col-lg-6 col-md-6 col-sm-6">
                                    <div class="contact-form">
                                        ...
                                        <select class="select2" name="center_slug">
                                            <option value="" selected disabled>Select an option</option>
                                        </select>
                                    </div>
                                </div>
                                    ...
                                    <div class="contact-form">
                                        ...
                                        <select class="select2" name="nationality">
                                            ...
                                            @foreach(nationality() ?? [] as $index => $item)
                                                <option {{old('nationality') === $index ? 'selected' : ''}} value="{{ $index }}" selected>{{ $item }}</option>
                                            @endforeach
                                        </select>
                                    </div>
                                </div>
...
@section('scripts')
    <script>
        $(document).ready(function () {
            ...
            if (city_id) {
                preSelectMedicalCenter(city_id, center_slug);
            }
        });

        const preSelectMedicalCenter = (city_id, center_slug) => {
            let centerList = @json(slipCenterListWithRate());
            let center = centerList[city_id];

            let centerName = $('select[name=center_slug]');
            centerName.empty();
            centerName.append('<option value="" selected disabled>Select a medical center</option>');

            $.each(center.list, function (index, item) {
                if(center?.rates[index].rate < 1) {
                    return;
                }

                centerName.append(`<option value="${index}">${item}</option>`);
            });

            if (center_slug) {
                centerName.val(center_slug);
            }
        }
